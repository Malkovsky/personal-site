---
title: "Extremely Fast and Succinct RMQ with Wide Segment Trees"
description: "This article describes a cutting edge implementation of the data structures for the range minimum queries -- a fundamental problem in string processing, compression and search indecies. These implementations are extremely fast but yet succinct, i.e. storing very low amount of auxiliary memory solve the problem."
author: "Nikolay Malkovsky"
date: "2026-06-28"
categories: [C++, algorithms, succinct data structures, RMQ, SIMD]
image: "images/wide_segment_tree.svg"
draft: false
...
- one that uses $1.05n$ auxiliary bits per input value and sometimes accesses the
  original array;
- one that uses $2.1n$ auxiliary bits per input value and never accesses the
  original array.
...
The local-min lookup gives the best depth reached inside each nibble. For this
query, the right boundary cuts nibble `3` down to only its first bit, so the
full-nibble high-lane value `0` is replaced by the partial value `+1`.

```text
lo_local_min:       [ +1, 0 ]     // 1110 has min +1; 1100 has min 0
hi_local_min full:  [ -2, 0 ]     // 0100 has min -2; 1010 has min 0
hi_local_min used:  [ -2,+1 ]     // nibble 3 only uses its first bit: 1
```
...
The low nibble starts at the byte boundary. The high nibble starts after the
low nibble, so its base also includes `lo_delta`:
...
For `[4, 13)`, the boundary chunks can be processed separately from the SIMD
middle. Conceptually, the active part of the four nibbles is:


```text
chunk:                 0       1        2       3
bits:               1110    0100     1100    1010
query mask:         0000    1111     1111    1000
```

After inactive lanes are replaced with the sentinel `127`, the candidate vectors
...
First compare the low and high candidate in each byte lane:
...
Then reduce the 16 signed byte lanes to one scalar minimum, using the same
doubling pattern as the prefix-sum code:
...
After shifts by `8`, `4`, `2`, and `1`, lane `0` contains the minimum over all
16 lanes. Here:
...
To recover the first nibble attaining this value, compare candidates with
`candidate_min` and convert the first set bit in each mask back to the original
nibble numbering:

```text
masked_lo == 0: [false, true ]  => low lane 1  => nibble index 2
masked_hi == 0: [true,  false]  => high lane 0 => nibble index 1
first winning nibble index = min(2, 1) = 1
```

Nibble `1` is `0100` with minimum excess `-2` at position `4`, so the first
minimum reached inside the block is:

```text
offset = 4 * nibble_index + local_offset
       = 4 * 1 + 4
       = 8
```

This is also the first interior zero-depth position in the BP sequence. In the
query `[4, 13)`, position `12` also attains depth `0`, but first-minimum
tie-breaking keeps position `8`. Therefore `RMQ(0, 6)` returns
`rank0(8) - 1 = 3`, which is the position of `A[3] = 1`.
